...
처음 버튼을 누르게되면 `count가` 갱신되므로 `Counter` 컴포넌트 `Child` 컴포넌트 둘 다 리렌더링됩니다. 버튼의 값도 0 에서 다른 값으로 변경됩니다. 그다음 버튼을 누르게되면 그 `obj.count`의 값은 변경이 되었지만 `Object.is`를 통해 이전값과 새로운 값을 비교해 보았을때, `obj`로 그 값이 동일하므로 버튼의 값은 변경되지 않습니다. 하지만 `Rendering` 이라는 값이 출력됩니다. 자식은 렌더링 되지 않습니다. 다시한번 버튼을 누르게 되면 `count`만 출력이되고 `Rendering`은 출력 되지 않습니다.
